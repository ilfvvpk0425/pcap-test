...
int main(int argc, char* argv[]) {
	...
	while (true) {
		...
		if (res == PCAP_ERROR || res == PCAP_ERROR_BREAK) {
	        ...
	        u_char* payload = (u_char*)(packet + sizeof(struct libnet_ethernet_hdr) + ip->ip_hl * 4);
	        ...
	        u_int payload_size = header->caplen - sizeof(struct libnet_ethernet_hdr) - ip->ip_hl * 4;
		if(payload_size <= 8) {
